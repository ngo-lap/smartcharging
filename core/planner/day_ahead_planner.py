...
from core.utility.data.data_processor import generate_mobility_data, prepare_planning_data
...
def create_optim_input(
        data_mobility: pd.DataFrame, horizon_length: int, time_step: int,
    ...
    if nbr_vehicle == len(data_mobility):
        logger.info("Planner called")
    else:
        logger.error(f"Number of vehicles: {nbr_vehicle} but data length is {len(data_mobility)}")

    # Data preparation
    arrival = data_mobility.loc[:, "arrivalTime"].tolist()
    departure = data_mobility.loc[:, "departureTime"].tolist()
    power = data_mobility.loc[:, "powerNom"].tolist()
    duration = data_mobility.loc[:, "chargingDuration"].tolist()
    energyRequired = data_mobility.loc[:, "energyRequired"].tolist()
    energyMax = data_mobility.loc[:, "energyMax"].tolist()
    ...
    if formulation == "milp":
        ...
        profile, totalPower, startTime = evcsp_milp(
        ...
        profile, totalPower, startTime = evcsp_lp(
    ...
    return profile, totalPower, startTime
...
if __name__ == '__main__':
    ...
    # Data Preparation
    data_mobility = generate_mobility_data(
        nbr_vehicles=nVE, horizon_length=horizon_length, time_step=time_step
    )
    data_planning = prepare_planning_data(data_mobility, time_step=time_step)

    # Planning
    profile, totalProfile, prob = create_optim_input(
